...
const coaches = [
  { name: "Paul", dupr: "4.5+", bio: "Paul has been playing pickleball for 6 or 7 years, plays nearly every single day, and LOVES it. In his spare time, he has a wife and four kids and runs a wealth management firm." },
  { name: "Heather", dupr: "3.25", bio: "Married to her husband of 25 years and mom of four. Heather loves Jesus and loves watching her kids play the sports they love. She works in the WXHS Teen Center and finds it very rewarding. A native Texan, she has lived in Utah for seven years." },
  { name: "JT", dupr: "3.9", bio: "JT is from Washington State, where he played football growing up, and later attended BYU. He has worked in collegiate athletics for the past five years and is married with a one-and-a-half-year-old daughter." },
];
  ...
  return (
    <details className={`schedule-block ${group.className}`} open>
      <summary className="schedule-team-title" style={{cursor:"pointer",listStyle:"none",display:"flex",alignItems:"center",justifyContent:"space-between",gap:20}}>
        ...
        <span className="schedule-toggle" aria-hidden="true">
          <span className="toggle-label toggle-label-open">Collapse</span>
          <span className="toggle-label toggle-label-closed">Expand</span>
          <svg className="schedule-chevron" width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"><path d="m6 9 6 6 6-6"/></svg>
        </span>
      </summary>
      <div className="schedule-table-wrap">
        <table className="schedule-table">
          <thead><tr><th>Date</th><th>Time</th><th>Event</th><th>Location</th><th>Players</th></tr></thead>
          <tbody>{group.games.map((g, i) => <tr key={`${group.name}-${g.date}-${g.title}-${i}`}><td>{g.day.charAt(0)+g.day.slice(1).toLowerCase()} | {g.date.charAt(0)+g.date.slice(1).toLowerCase()}, {g.year}</td><td>{g.time}</td><td>{g.title}</td><td>{g.location}</td><td><span className={`player-badge ${g.players === "ALL" ? "all" : g.players === "Development" ? "dev" : "team"}`}>{g.players}</span></td></tr>)}</tbody>
        </table>
      </div>
    </details>
  );
...
export default function Home() {
  ...
  return <><style>{`.schedule-team-title::-webkit-details-marker{display:none}.schedule-toggle{display:flex;align-items:center;gap:8px;font-size:13px;font-weight:900;letter-spacing:.04em;text-transform:uppercase;color:#fff;flex-shrink:0}.schedule-chevron{transition:transform .2s ease}.schedule-block[open] .schedule-chevron{transform:rotate(180deg)}.schedule-block .toggle-label-open{display:none}.schedule-block .toggle-label-closed{display:inline}.schedule-block[open] .toggle-label-open{display:inline}.schedule-block[open] .toggle-label-closed{display:none}.site-menu{position:relative;margin-left:auto}.site-menu>summary{list-style:none;cursor:pointer;width:48px;height:48px;border:1px solid rgba(255,255,255,.28);border-radius:9px;display:grid;place-items:center;background:rgba(255,255,255,.06)}.site-menu>summary::-webkit-details-marker{display:none}.hamburger{width:22px;height:16px;display:flex;flex-direction:column;justify-content:space-between}.hamburger span{display:block;height:2px;background:#fff;border-radius:2px;transition:.2s ease}.site-menu[open] .hamburger span:nth-child(1){transform:translateY(7px) rotate(45deg)}.site-menu[open] .hamburger span:nth-child(2){opacity:0}.site-menu[open] .hamburger span:nth-child(3){transform:translateY(-7px) rotate(-45deg)}.site-menu-panel{position:absolute;right:0;top:58px;min-width:250px;background:#071c31;border:1px solid rgba(255,255,255,.18);border-radius:12px;box-shadow:0 18px 40px rgba(0,0,0,.3);overflow:hidden;padding:8px}.site-menu-panel a{display:flex;align-items:center;justify-content:space-between;min-height:50px;padding:0 15px;border-radius:8px;color:#fff;font-size:13px;font-weight:900;letter-spacing:.06em;text-transform:uppercase}.site-menu-panel a:hover{background:rgba(255,255,255,.08)}.coaches-section{background:#f4f7fa}.coaches-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:22px;margin-top:36px}.coach-card{background:#fff;border-radius:16px;padding:30px;border:1px solid #e1e7ed;border-top:5px solid #e51f35;box-shadow:0 12px 35px rgba(7,28,49,.07)}.coach-card h3{font-size:30px;margin:0 0 8px}.coach-dupr{display:inline-block;color:#e51f35;font-size:12px;font-weight:900;letter-spacing:.1em;text-transform:uppercase;margin-bottom:18px}.coach-card p{color:#657487;line-height:1.65;margin:0;font-size:16px}@media(max-width:800px){.coaches-grid{grid-template-columns:1fr}}@media(max-width:560px){.schedule-toggle{font-size:11px;gap:5px}.schedule-chevron{width:24px;height:24px}.site-menu-panel{position:fixed;right:14px;left:14px;top:76px;min-width:0}.site-menu>summary{width:44px;height:44px}}`}</style><header className="topbar"><div className="container nav"><a className="brand" href="#home"><img className="brand-logo" src={LOGO} alt="WX Pickleball logo"/><div className="brand-copy"><strong>WX PICKLEBALL</strong><span>Woods Cross High School</span></div></a><details className="site-menu"><summary aria-label="Open site menu"><span className="hamburger" aria-hidden="true"><span></span><span></span><span></span></span></summary><nav className="site-menu-panel"><a href="#home">Home <span>→</span></a><a href="#team">Team <span>→</span></a><a href="#coaches">Coaches Corner <span>→</span></a><a href="#schedule">Schedule <span>→</span></a><a href="/tournament">Fundraiser <span>→</span></a><a href="#contact">Contact <span>→</span></a></nav></details></div></header><main><section className="hero" id="home"><div className="container hero-grid"><div className="hero-inner"><div className="kicker">Woods Cross High School Pickleball</div><img className="hero-team-photo" src="/IMG_2759.jpg" alt="WX Pickleball team"/><div className="actions"><a className="btn btn-primary" href="#schedule">View Schedule</a><a className="btn btn-secondary" href="#team">Meet the Team</a></div></div><div className="hero-logo-wrap"><img className="hero-logo" src={LOGO} alt="WX Pickleball wildcat logo"/></div></div></section><section className="section" id="team"><div className="container"><div className="kicker">Our Goals</div><h2>Our Focus</h2><div className="grid grid-4 goals-grid" style={{marginTop:36}}><div className="card"><div style={iconStyle}><FocusIcon type="development"/></div><h3>Development</h3><p>Our goal is to help the athletes develop from wherever they are. We focus on fundamental game play, technical skills, and strategies to win.</p></div><div className="card"><div style={iconStyle}><FocusIcon type="character"/></div><h3>Character</h3><p>Pickleball rewards positive thinking, self-confidence, and attitude. It’s also an extremely social sport. We feel these are wonderful life lessons.</p></div><div className="card"><div style={iconStyle}><FocusIcon type="competition"/></div><h3>Competition</h3><p>We want to compete at the highest levels in the state.</p></div><div className="card"><div style={iconStyle}><FocusIcon type="fun"/></div><h3>Fun</h3><p>“I’ll say this about WX pickleball, no other school is having as much fun out there as you guys are!”</p><p style={{marginTop:14,fontWeight:800}}>— American Heritage Head Coach</p></div></div></div></section><section className="photo-section" id="photos"><div className="container"><div className="photo-heading"><div className="kicker">WX Pickleball</div><h2>This is our team.</h2><p className="section-lead">Competition, teammates, tournaments, and the moments that make the program.</p></div><div className="photo-showcase"><figure className="photo-feature"><img src="/IMG_2759.jpg" alt="WX Pickleball full team at a tournament"/></figure><div className="photo-grid"><figure><img src="/IMG_2473.jpg" alt="WX Pickleball team on court"/></figure><figure><img src="/IMG_3092.jpg" alt="WX Pickleball players at The Picklr"/></figure><figure><img src="/IMG_3237.jpg" alt="WX Pickleball team at an indoor event"/></figure><figure><img src="/IMG_2976.jpg" alt="WX Pickleball players on the podium"/></figure></div></div></div></section><section className="section coaches-section" id="coaches"><div className="container"><div className="kicker">Meet the Coaches</div><h2>Coaches Corner</h2><p className="section-lead">The people helping our players develop, compete, and enjoy the game.</p><div className="coaches-grid">{coaches.map(coach=><article className="coach-card" key={coach.name}><h3>{coach.name}</h3><div className="coach-dupr">EST. DUPR: {coach.dupr}</div><p>{coach.bio}</p></article>)}</div></div></section><section className="section" id="fundraiser"><div className="container"><div className="cta"><div><div className="kicker dark-kicker">August 22 • Hogan Park</div><h3>WX Fundraising Tournament</h3><p>Grab a partner, compete, and help support the Woods Cross High School Pickleball program.</p></div><a className="btn cta-btn" href="/tournament">Tournament Details</a></div></div></section><section className="section schedule-section" id="schedule"><div className="container"><h2 className="schedule-main-title">2026-2027 Schedule</h2><div className="schedule-groups">{scheduleGroups.map(group=><ScheduleTable group={group} key={group.name}/>)}</div><div className="schedule-note"><strong>Schedules are subject to change.</strong> Check back regularly for updates. All times Mountain Time (MT).</div></div></section><section className="section" id="contact"><div className="container"><div className="cta"><div><div className="kicker dark-kicker">WX PICKLEBALL</div><h3>Ready to play or support the team?</h3><p>Registration, volunteer, sponsor, and contact links will live right here.</p></div><a className="btn cta-btn" href="mailto:">Contact WX Pickleball</a></div></div></section></main><footer className="footer"><div className="container footer-inner"><span>© 2026 WX Pickleball • Woods Cross, Utah</span><span>Compete • Improve • Represent</span></div></footer></>;
}
