...
const scheduleGroups = [
  { name: "Development", note: "Development open play plus all-program events.", games: games.filter((g) => g.players === "Development" || g.players === "ALL") },
  { name: "JV", note: "JV practices, meets, championships, and all-program events.", games: games.filter((g) => ["JV", "JV & Varsity", "ALL"].includes(g.players)) },
  { name: "Varsity", note: "Varsity practices, meets, championships, and all-program events.", games: games.filter((g) => ["Varsity", "JV & Varsity", "ALL"].includes(g.players)) },
];

function ScheduleList({ group }) {
  return (
    <div className="schedule-group">
      <div className="schedule-group-head">
        <div><div className="kicker">{group.name}</div><h3>{group.name} Calendar</h3><p>{group.note}</p></div>
        <div className="schedule-count">{group.games.length} events</div>
      </div>
      <div className="schedule">
        {group.games.map((g, i) => (
          <div className="game" key={`${group.name}-${g.date}-${g.title}-${i}`}>
            <div className="game-date">{g.date}<small style={{display:"block",fontSize:11,marginTop:4}}>{g.day}</small></div>
            <div className="game-title">{g.title}<small style={{display:"block",fontSize:13,fontWeight:500,marginTop:5}}>{g.time}</small></div>
            <div className="game-meta">{g.location}<br/><strong>{g.players}</strong></div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <>
      ...
      <main>
        ...
        <section className="section schedule-section" id="schedule"><div className="container"><div className="kicker">2026–27 Season</div><h2>Three teams. Three calendars.</h2><p className="section-lead">Development, JV, and Varsity each have their own schedule. All-program events appear on all three calendars.</p><div className="schedule-groups">{scheduleGroups.map((group) => <ScheduleList group={group} key={group.name} />)}</div></div></section>
